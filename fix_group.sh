...
from tasks import assign_tasks, update_streak
...
async def cmd_referral(message: types.Message):
    if message.chat.type != "private":
        await message.answer("👥 Реферальная программа работает в личных сообщениях с ботом. Напиши мне в лс: @TradeAll_bot")
        return
    user_id = message.from_user.id
...
async def cmd_tasks(message: types.Message):
    if message.chat.type != "private":
        await message.answer("📋 Задания доступны в личных сообщениях с ботом. Напиши мне: @TradeAll_bot")
        return
    user_id = message.from_user.id
...
# Обновляем gamification.py чтобы розыгрыш слал уведомление в группу
cat > /tmp/group_update.py << 'EOF'
import re

with open('/opt/bot/gamification.py', 'r') as f:
    content = f.read()

# Добавляем импорт GROUP_ID
content = content.replace(
    'from config import LEVELS, EXP_SOURCES, ACHIEVEMENTS, WEEKLY_PRIZES',
    'from config import LEVELS, EXP_SOURCES, ACHIEVEMENTS, WEEKLY_PRIZES\nGROUP_ID = -1003715048435'
)

# Добавляем отправку в группу после розыгрыша
old = "for wid in winners:\n        try:\n            await bot.send_message(wid, f\"🎉 Вы выиграли {prize} дней подписки!\")\n        except:\n            pass\n    return text"
new = """try:
...
print("gamification.py обновлён")
EOF

python3 /tmp/group_update.py
